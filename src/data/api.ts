...
/**
 * Saves invoice fields to Firestore and records an AI self-healing rule
 * when the vendor name is manually corrected.
 */
async function saveInvoiceToDb(invoiceId: string, data: Partial<Invoice>): Promise<void> {
    ...
    // AI Self-Healing Loop: log the manual vendor correction as a global rule
...
/**
 * Saves the corrected invoice as a ground-truth example for the Teacher Agent
 * (few-shot learning via invoice_examples collection).
 */
async function saveTeacherExample(invoiceId: string, vendorName: string, d: Record<string, any>): Promise<void> {
...
/**
 * Updates the vendor profile with correction statistics and auto-generates
 * Charter rules when consistent corrections reach the threshold (2+).
 */
async function updateVendorProfile(vendorName: string, d: Record<string, any>): Promise<void> {
...
/**
 * Extracts and stores universal learning patterns (VAT→currency, payment terms,
 * tax rates) into the teacher_global_rules collection. Applies to ALL invoices.
 */
async function generateGlobalRules(d: Record<string, any>): Promise<void> {
...
/**
 * Checks recent bank transactions for a payment matching this invoice.
 * Uses strict rules: reference match + vendor overlap + amount tolerance ±0.50.
 * Marks the invoice as Paid and the transaction as matched when found.
 */
async function reconcileWithBankStatement(invoiceRef: ReturnType<typeof doc>, d: Record<string, any>): Promise<void> {
    if (!d.companyId || (d.status === 'Paid' && !(d.currency && d.currency !== 'EUR'))) return;
...
/**
 * Orchestrates a full invoice update: persists changes, runs Teacher Agent
 * learning, updates vendor profile, generates global rules, and reconciles
 * with bank statements. External interface is unchanged.
 */
export const updateInvoice = async (invoiceId: string, data: Partial<Invoice>): Promise<void> => {
    if (!db) throw new Error("Database not initialized");

    await saveInvoiceToDb(invoiceId, data);

    // Fetch saved invoice once — shared by Teacher, vendor profile, global rules, and reconciliation
    const invoiceRef = doc(db, 'invoices', invoiceId);
    const snap = await getDoc(invoiceRef);
    ...
    try {
        await reconcileWithBankStatement(invoiceRef, d);
    } catch (reconErr) {
