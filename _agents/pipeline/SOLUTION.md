...
Весь код реализован. Пайплайн заблокирован до получения Dropbox credentials от Einar.
...
8. Добавить в Railway → Variables:
   - `DROPBOX_ACCESS_TOKEN` = скопированный токен
9. После деплоя проверить: `node automation/dropbox_service.cjs --test`

## После получения токенов

1. Добавить токен в Railway
2. Задеплоить
3. Запустить `node automation/dropbox_service.cjs --test`
4. Убрать Zapier webhook (`/api/intake`) из `webhook_server.cjs` если Zapier больше не используется
