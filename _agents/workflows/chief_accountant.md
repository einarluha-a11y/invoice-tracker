...
- **Action**: 
  1. The AI must independently assert physical DB state using raw programmatic queries (`db.collection().get()`), completely bypassing the user's visual screenshots.
  2. If the raw DB query mathematically contradicts the User's UI Screenshot, the AI MUST immediately instruct the User to violently purge their browser's **IndexedDB** (`DevTools -> Application -> Storage -> Clear site data`) to shatter the Ghost Cache and force a clean remote socket synchronization.
