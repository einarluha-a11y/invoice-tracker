...
## 1. THE EXTRACTION MANDATE (CORE FIELDS)
- **Mandate**: When pulling any invoice from an email inbox, you MUST extract the following essential fields without fail: **Company Name (Vendor)**, **VAT Number**, **Registration Code**, **Invoice Description** (what they are asking money for), **Amount without tax (Subtotal)**, **Tax amount**, and **Total amount with tax**.

## 2. THE CHARTER INITIALIZATION RULE
- **Mandate**: You must ALWAYS start the process of viewing or analyzing any invoice by reading this ENTIRE Chief Accountant Charter from top to bottom before taking any action.

## 3. THE FILE PRESERVATION PROTOCOL
- **Mandate**: The physical invoice file (PDF/Image) itself MUST always be saved in the system and definitively attached to its corresponding active database record. Never create a record without its physical file backing it up.

## 4. The "European Comma" Rule (Floating Point Normalization)
...
## 5. The "Ettemaksuteatis" Rule (Proforma JUNK Rejection)
...
## 6. The PM2 Memory Cache Protocol
...
## 7. Strict Workspace Compartmentalization
...
## 8. Zero-Amount Fileless Hallucinations
...
## 9. PROACTIVE EXECUTION ALGORITHM
...
## 10. THE "Записывай в Устав" KEYWORD (Dynamic Self-Update Protocol)
...
## 11. THE RECURRING HISTORICAL SUBSTRING TRAP (Priority Queue Mandate)
...
## 12. THE KREEDITARVE (CREDIT NOTE) PROTOCOL
...
## 13. THE CROSS-COMPANY ROUTING & SPAM PROTOCOL
...
## 14. THE FUZZY DEDUPLICATION PROTOCOL (ANTI-CLONE SHIELD)
...
## 15. THE PRE-PAID RECEIPT PROTOCOL (RETAIL & RIDE-SHARING)
...
## 16. THE CROSS-CURRENCY SETTLEMENT PROTOCOL (FX OVERWRITE)
...
## 17. THE OVERSEER PROTOCOL (INDEPENDENT PM2 WATCHDOG)
...
## 18. THE LIVE-FIRE VERIFICATION PROTOCOL (SYSTEMIC AUDITS)
...
## 20. THE RELATIONAL TEMPORAL PROTOCOL (Päeva Jooksul)
...
## 21. THE ABSOLUTE DATE FALLBACK (RECEIPTS)
...
## 22. THE HISTORIC PROFORMA OVERRIDE (IMAP ARCHIVE PIERCING)
...
## 23. THE SYNONYMOUS MERCHANT PROTOCOL (ALIAS REGISTRY)
...
## 24. THE COMPOUNDING DEBT TRAP (VÕLGNEVUS ISOLATION)
...
## 25. THE STRICT TYPE PREDICATE (ZERO-SUM BYPASS)
...
## 26. THE BALTIC INVOICE FIELD EXTRACTION PROTOCOL
...
## 27. THE SUPREME SUPERVISOR HALLUCINATION BAN
...
## 28. THE BODY-TEXT EMAIL AUDIT PROTOCOL
...
## 29. THE EMAIL IDEMPOTENCY PROTOCOL (MARK-AFTER-WRITE)
...
## 30. THE PM2 WATCH ISOLATION PROTOCOL (RESTART LOOP PREVENTION)
...
## 31. THE FIREBASE gRPC ISOLATION PROTOCOL
...
## 32. THE GOVERNMENT SOURCE LOOKUP PROTOCOL (FALLBACK ENRICHMENT)
...
## 33. THE PRIVATE PERSON & POLISH COMPANY PROTOCOL
...
## 34. THE FILE ATTACHMENT GUARANTEE
...
## 35. THE IDENTICAL SUM FALLACY & IMAP HORIZON CRASH
...
## 37. THE UNIVERSAL GOVERNMENT SOURCE PROTOCOL (GLOBAL "GOV VERIFIED")
...
## 38. THE IMAP RATE LIMIT PROTOCOL (THROTTLING)
...
## 39. THE UI GHOST CACHE PROTOCOL (INDEXEDDB DESYNC)
- **The Error**: The AI backend agent repairs damaged invoices (e.g. restoring `fileUrl`, changing status) directly via the Firebase Admin SDK, but the Desktop PWA UI completely refuses to render the changes (e.g. missing 📄 icons), leading the AI to hallucinate non-existent CSS Flexbox width collapsing bugs.
- **The Root Cause**: The React Frontend (`src/firebase.ts`) explicitly deploys `localCache: persistentLocalCache()`. This IndexedDB abstraction fiercely caches the older schema of the deeply nested documents. Even if the Vercel Service Worker is cleared via `npm run deploy`, the local browser database intercepts the Firebase `onSnapshot` listener and actively serves the stale, broken "Ghost Cache" to the UI, completely ignoring the pure Admin SDK data sitting on the cloud.
- **Mandate**: NEVER assume the frontend UI truthfully represents actual Firestore state after executing backend administrative data patches.
- **Action**: 
  1. **Ban CSS Hallucinations**: Do not hallucinate CSS flexbox collapsing bugs or padding issues to explain missing UI elements.
  2. **Verify Frontend Logic**: Check `parseStatus()` and all data mappings in `src/data/api.ts` first. The visual state in the UI is highly transformed, not raw data.
  3. **Assert Database Reality**: Programmatically verify the raw Firestore fields (`db.collection().get()`) against what the User's UI screenshot displays. **CRITICAL**: The diagnostic script MUST be written by the AI but executed by the USER locally from their terminal (`cd automation && node script.cjs`). Due to network proxy restrictions in the AI's sandbox environment, running the script internally often results in silent timeouts or Exit Code 143, leading the AI to falsely conclude the database is empty.
  4. **Purge Ghost Cache**: Only if the raw Firestore data is mathematically correct and the frontend code is flawless, but the visual mismatch persists, MUST the AI instruct the User to violently purge their browser's **IndexedDB** (`DevTools -> Application -> Storage -> Clear site data`) to force a clean remote socket synchronization.
