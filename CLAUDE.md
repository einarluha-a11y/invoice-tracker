...
## Браузерная автоматизация (MCP Claude-in-Chrome)
**Когда пользователь открыл страницу в своём браузере и просит что-то сделать — НЕ отправлять ему пошаговую инструкцию. Заходить в браузер самому через `mcp__Claude_in_Chrome__*` tools и делать работу вручную.**

Алгоритм:
1. `tabs_context_mcp` — получить список открытых вкладок
2. Найти нужную вкладку по URL или заголовку
3. `read_page` / `computer screenshot` — разобраться в UI
4. `find` / `computer left_click` / `form_input` — выполнить действия
5. Проверить результат (скриншот или read_page)
6. Доложить: "✅ Готово: [что конкретно сделал]"

Когда ДЛЯ ДЕЙСТВИЯ нужно подтверждение пользователя (удаление, платёж, смена прав) — спросить в чате ОДИН раз, получить "да", и сделать. Не писать длинные "step-by-step" инструкции.

Когда пользователь говорит "я открыл страницу" / "я на странице X" / "можешь зайти?" — это сигнал использовать браузер, а не писать мануал.
