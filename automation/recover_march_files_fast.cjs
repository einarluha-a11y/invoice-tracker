...
async function recoverMailboxFast(config, missingDocs, label) {
    ...
    try {
        ...
        console.log(`Fetching headers since March 1, 2026 for ${label}...`);
        const messages = await connection.search(searchCriteria, fetchOptions);
        console.log(`Found ${messages.length} recent headers.`);
        ...
        for (const target of missingDocs) {
            ...
            for (const item of messages) {
                const headerRaw = item.parts.find(a => a.which === 'HEADER');
                const header = headerRaw.body;
                ...
                if (subject.toLowerCase().includes(target.keyword.toLowerCase()) || from.toLowerCase().includes(target.keyword.toLowerCase())) {
                    console.log(`  -> Header match: "${subject}" from ${from}. Fetching full body UID: ${item.attributes.uid}...`);
                    
                    // Fetch full body exactly for this UID
                    const fullMsg = await connection.search([['UID', item.attributes.uid]], { bodies: [''], struct: true });
                    if (fullMsg.length > 0) {
                        const allBody = fullMsg[0].parts.find(a => a.which === '');
                        const parsedEmail = await simpleParser(allBody.body);
...
async function run() {
    await recoverMailboxFast(IC_CONFIG, missingIdeacom, 'Ideacom');
    await recoverMailboxFast(GT_CONFIG, missingGT, 'Global Technics');
