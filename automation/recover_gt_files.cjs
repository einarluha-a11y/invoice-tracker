...
async function recoverMissingFiles() {
    const missingDocs = [
        { id: 'N8LBYglgOIQA13QiMLUa', vendor: 'Allstore', keyword: 'Kookon' }
    ];
    ...
    try {
        ...
        // Look at messages from March 1st onwards
        const searchCriteria = [['ALL']];
        const fetchOptions = { bodies: ['HEADER', ''], struct: true };
        
        console.log("Fetching emails since March 1st...");
                    ...
                    if (parsedEmail.attachments && parsedEmail.attachments.length > 0) {
                        for (const attachment of parsedEmail.attachments) {
                            if (attachment.contentType.includes('pdf') || attachment.filename.endsWith('.pdf')) {
                                ...
                                foundPdf = true;
                                break; // Break out of attachment loop
                            }
                        }
                    }
                    if (foundPdf) break; // Break out of messages loop once we find ONE valid PDF for this vendor
                }
            }
            if (!foundPdf) {
